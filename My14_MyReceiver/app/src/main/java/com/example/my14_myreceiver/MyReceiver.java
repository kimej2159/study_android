...
public class MyReceiver extends BroadcastReceiver {
    ...
    //데이터의 날짜 형태를 내가 원하는 형태로 변형시키기 위한 변수 선언
    public SimpleDateFormat dateFormat =
            new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");

    @Override
    //                   Context : 메인 환경 설정 파일, intent: 메세지 수신 데이터,
    public void onReceive(Context context, Intent intent) {
        Log.d(TAG, "onReceive: 호출됨");
    // Bundle : 내부적으로 데이터를 저장할 수 있는 객체
        ...
        if(messages !=null && messages.length > 0){
            ...
            //인텐트를 만들어서 데이터를 넣은 후 액티비티를 띄운다
            Intent disIntent = new  Intent(context, SmsDisActivity.class);
            // 액티비티가 아닌 곳에서 인텐트를 만들어 새창을 띄우고자 할 때
            // FLAG_ACTIVITY_NEW_TASK를 플래그에 추가해야 함
            disIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
                    //SINGLE_TOP은 똑같은 화면이 이미 띄워져 있는 상태라면 그 화면을 재사용 하겠다
                    // 재사용하는 화면에 데이터 갱신을 위햄 on NewIntent를 오버라이드 해야함
                    // 오버라이드 해서 거기서 작업을 한다
            ...
            //액티비티가 아닌 경우는 context가 없이 새창을 띄울 수 없다
    ...
    //bundle 객체에 있는 데이터 중 메세지만 피싱하여 리턴시킴
